...
| `marine_bathymetry_store` | C++ | Persistent multi-source bathymetric data store: GGGS-tiled, priority source layers, best-source / shallowest-reliable queries, per-tile GeoTIFF (ADR-0002 / #86); `s102_import` CLI fetches + converts NOAA S-102 tiles (#278); `build_depth_overviews` builds the single-band pyramid for `draft/processed/reference`, and `build_depth_overview_parent` folds one tile of the rev-3 **4-band MIN/MEAN/COUNT/σ** pyramid for a world-store quantity layer (#397) — rev 3 replaces the single-band tree (no compatibility kept; it is wiped and rebuilt once rev 3 works), and until then each writer refuses to replace the other's sidecar |
...
| `marine_world_store` | Python | World store (rev 3, `docs/world_store_design.md`) foundation: store-root resolution (one `_DEFAULT_ROOT`, grep-enforced), the `<quantity>/<state>/<origin>/` layout, content-based source identity (Merkle bag id, §3), the observation interval every Item is dated by — read from the source bags' `metadata.yaml`, union over a product's sources, and a named refusal (nothing written) when it cannot be derived — product fingerprints (§9), STAC Items/Collections with the Part 2 consumer-contract fields (overview tiles' Items built from their lineage), append-only `revisions/` records, and the `mws_*` CLIs including the byte-identical native-tile adapter `mws_link_depth_subset`, the σ-fold candidate measurement `mws_measure_sigma_fold` (design §7's rule is **open**; the measurement recommends nothing) and the §9 regenerate workflow under `snakemake/` (fingerprint pre-step at load, real-file inputs, per-level prune, one tile index per band schema) (#397). It replaces the `draft/processed/reference/chart` tree rather than coexisting with it (no compatibility kept; the existing stores are wiped and rebuilt once rev 3 works) and reads that tree only as the adapter's input. A **plain Python package with a `package.xml` shim** — no `rclpy`/`ament` imports, enforced by tests; declares repo-local rosdep keys in the root `rosdep.yaml`. |
...
│   │   │                       #   fingerprints, source time, Items, revisions,
...
├── rosdep.yaml                 # Repo-local rosdep keys (ws#654 policy)
