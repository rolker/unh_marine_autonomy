...
### Findings
- [ ] (must-fix) `_on_fix` sets `self._fix = msg` before the newest-stamp guard, so an out-of-order fix from one of the multiple position subs is published, jumping the public point backward — violates the stated "newest wins" contract — `marine_web_view/marine_web_view/state_renderer.py:250`
- [ ] (suggestion) `decimate_track` has no catch-all band; points older than the hardcoded 14400 s are dropped even when `track_seconds` is set larger, so a longer configured track never renders — `marine_web_view/marine_web_view/state_renderer.py:74,136`
- [ ] (suggestion) Dry-run local writes are non-atomic (`open('w')`); the local http.server preview can read truncated JSON mid-write — use tmp+os.replace — `marine_web_view/marine_web_view/state_renderer.py:419,457`
