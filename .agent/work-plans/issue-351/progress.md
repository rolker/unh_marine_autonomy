...
## Integrated Review
**Status**: complete
**When**: 2026-08-25 09:06 -04:00
**By**: Claude Code Agent (Claude Opus 5 (1M context))

**PR**: #353 at `89f556f`
**Sources**: 7 (Copilot R1 @ `a75dec3`, R2 @ `ce77d90`, R3 @ `196dac2`, R4 @ `b7d3beb`, R5 @ `89f556f`; Local Review (Pre-Push) R3 @ `196dac2` and R4 @ `b7d3beb`; CI rollup)
**Cross-source confirmations**: 3
**CI**: all-pass (build success, copilot-pull-request-reviewer success)

### Findings
- [x] (cross-confirmed: Copilot R2 + R3, Local Review R4 @ `b7d3beb`) `main()` constructs `StateRenderer()` outside its `try/finally`, so a constructor raise (`ProfileNotFound`, a bad region, an SDK import failure — all newly reachable now that the client is built in `__init__`) leaves the rclpy context initialised with no `shutdown()`. `coverage_renderer.main()` already protects this and `test_the_render_worker_is_started_after_everything_that_can_raise` codifies the invariant for that node only — `marine_web_view/state_renderer.py:621-637`
- [x] (must-fix, Local Review R4 @ `b7d3beb`) `coverageText()` returns on any `status !== 'ok'` before the `age > COVERAGE_DEAD_S` branch, so a renderer that exited after a truncated flush reads `truncated render` forever with no age — and this PR makes `truncated_render` the common last word of a run. Only the layer opacity says the renderer is dead — `marine_web_view/web/index.html:523-531`
- [x] (must-fix, Local Review R4 @ `b7d3beb`) `_log_upload_failure` is the node's only unthrottled error path; a persistent `AccessDenied`/`NoSuchBucket` — the expected first-deployment failure on an operator station — logs one ERROR per second forever. Add `throttle_duration_sec=30.0`, matching `_queue`/`stop` — `marine_web_view/state_renderer.py:611-619`
- [x] (suggestion, Copilot R4 @ `b7d3beb`) `hashlib.md5()` defaults to `usedforsecurity=True` and raises `ValueError` on a FIPS-enforced OpenSSL build, aborting every chart refresh in the local comparison — for a digest that is explicitly only an ETag fingerprint. Pass `usedforsecurity=False` here and in the test helper — `marine_web_view/scripts/refresh_chart_tiles.py:619-625`
- [x] (cross-confirmed: Copilot R5 @ `89f556f`, Local Review R4 @ `b7d3beb`) `stop()`'s docstring now discloses the unbudgeted manifest PUT and in-flight request, but `WORKER_JOIN_SECONDS`'s own comment still says the join "only has to cover the request it is already inside" — since round 4 an aborted scheduled pass issues one more manifest PUT first — `marine_web_view/marine_web_view/coverage_renderer.py:164-172`
- [x] (cross-confirmed: Copilot R3 @ `196dac2`, Local Review R4 @ `b7d3beb`) two failure paths in the cron script leave `main()` as a bare traceback where every other path does `print(..., file=sys.stderr); return 1`: `remote_etags`'s `list_objects_v2` (an `AccessDenied` or transient list failure after the full ~1 h crawl) and `update_manifest`'s lock/strict-read raise. Exit status is already non-zero and the manifest is already withheld, so this is cron-mail presentation — `marine_web_view/scripts/refresh_chart_tiles.py:715`, `:928`
- [x] (suggestion, Local Review R4 @ `b7d3beb`) an abort during `_update_datum_offset()` or on `_render_pending`'s first check publishes `truncated_render` having rendered nothing (`rendered_tiles` unmoved); gate the publish on progress this pass, or narrow the test docstring claiming such a pass "has nothing to announce" — `marine_web_view/marine_web_view/coverage_renderer.py:1215-1226`, `test/test_render_pass.py:437-443`
- [x] (suggestion, Copilot R3 @ `196dac2`) the PR description's test plan still claims 161 tests / 19 mutations; the branch carries 200 test functions and the round-4 record reports 203 tests / 21 mutants killed. Update the PR body — `PR #353 body`

### False positives
- (Copilot R5 @ `89f556f`) `is_content_hash` admits an SSE-KMS ETag, so a KMS object "can compare equal and be skipped despite changed bytes" — that requires the KMS ETag to equal the local file's MD5, a 128-bit coincidence, not a reachable failure mode. Every other KMS ETag mismatches and is uploaded, which is the fail-safe direction the docstring already claims.
- (Copilot R4 @ `b7d3beb`) `AsyncUploader.submit` can accept after `stop()` sets `_stop` and return True for a payload the worker abandons — `submit`'s contract is "queued", not "sent"; `stop()`'s docstring already states that anything still pending is abandoned, and `confirmed()` is the caller's sent-signal. Nothing reads the accept as delivery.
- (Copilot R1 @ `a75dec3`, R2 @ `ce77d90`) no operation-level deadline on `put_object`, so retries can outlive the old 20/30 s subprocess cap — deliberate and documented in `_boto3_client`: no per-request ceiling exists (urllib3 applies `connect_timeout` per resolved address), and two earlier rounds wrote a specific number here and were wrong both times. Containment is structural instead: state_renderer PUTs on its own thread off the executor, coverage_renderer checks `abort()` between tiles, and the cron script is bounded by the run lock plus `SYNC_DEADLINE_SECONDS`.
- (Copilot R1 @ `a75dec3`, R3 @ `196dac2`) stale wording in `.agent/work-plans/issue-351/plan.md` and `progress.md` — planning artifacts; the plan was resynced in `8dd8cdb` and progress entries are an append-only record of what each round found, not a document kept current.
- (Copilot R2 @ `ce77d90`) `load_manifest`'s blanket except turns auth/transport failures into a missing manifest, and the per-name lock does not protect the shared `tiles/manifest.json` — both fixed in round 3: `strict=True` on the read-modify-write path and a shared manifest lock in `update_manifest`.

### Out of scope for this PR (follow-ups)
- (Copilot R3 @ `196dac2`, already flagged out-of-scope by Local Review R3) `local_files()` walks the persistent `--workdir` with symlink-following `open()`, so a planted symlink in a shared `/tmp/p11-tiles` can make the admin-profile job PUT an arbitrary local file to the public `tiles/` prefix. Pre-existing and larger than the staging-file fix round 3 landed — needs its own issue.
- (Copilot R3 @ `196dac2`) a failed or blank fetch leaves the previous run's PNG in the persistent `outdir`, and the 5 % failure gate lets it be republished as current. Pre-existing: `aws s3 sync` uploaded the same stale bytes, so the transport swap does not make it worse.
- `.agents/README.md` does not list `marine_web_view`, so the new `python3-boto3` dependency lands in a package the repo's agent guide does not describe — already tracked as #348.

## Implementation
**Status**: complete
**When**: 2026-08-25 09:17 -04:00
**By**: Claude Code Agent (Claude Opus)

**PR**: #353 at `6d6a6f2`
**Addressed**: Integrated Review (2026-08-25 09:06 -04:00, PR #353 @ `89f556f`) — all 8 open actions
**Commits**: 0d0ad5a, c2eb5c9, 722884a, a874b6f, 4a32ac1, 07d478d, 6d6a6f2 (+ PR body edit, no commit)

### Actions
- [x] `main()` constructs `StateRenderer()` outside its `try/finally` — moved inside, `node` guarded with `if node is not None`, matching `coverage_renderer.main()`; new behavioural test drives `main()` with a raising constructor and asserts `rclpy.shutdown()` still runs — `marine_web_view/marine_web_view/state_renderer.py:620-643`, `test/test_node_upload_wiring.py`
- [x] `coverageText()` returned on a non-ok status before consulting the age — the age is now decided first and prefixed to the status word (`stale — N s · truncated render`), so a renderer that exited after a truncated flush still reads as dead; textual guard added asserting `COVERAGE_DEAD_S` is reached before the status branch returns — `marine_web_view/web/index.html:522-541`, `test/test_page_layers.py`
- [x] `_log_upload_failure` unthrottled — `throttle_duration_sec=30.0`, matching `_queue`/`stop` — `marine_web_view/marine_web_view/state_renderer.py:611-625`
- [x] `hashlib.md5()` raises on a FIPS-enforced build — `usedforsecurity=False` in `file_md5` and at both test-helper sites — `marine_web_view/scripts/refresh_chart_tiles.py:619-625`, `test/test_chart_tile_sync.py:159,242`
- [x] `WORKER_JOIN_SECONDS`'s comment still claimed the join "only has to cover the request it is already inside" — rewritten to name the abort-path manifest PUT as well — `marine_web_view/marine_web_view/coverage_renderer.py:164-174`
- [x] two cron paths left `main()` as a bare traceback — `sync_dir` (its opening `remote_etags`) and `update_manifest` are now wrapped, each printing one `sys.stderr` line and returning 1 like every sibling path; added `_describe_error` (reads the S3 error code without importing botocore, like `_is_missing_object`) and a test per path asserting the line, the exit status and no traceback — `marine_web_view/scripts/refresh_chart_tiles.py:513-528,930-935,957-964`, `test/test_chart_tile_sync.py`
- [x] an abort that rendered nothing published `truncated_render` with the previous pass's counts — the announce is now gated on `self._rendered` having moved this pass, so a budget spent in `_update_datum_offset()` or before the first tile announces nothing (same outcome as the top-of-function abort); the `test_the_shutdown_flush_is_bounded` docstring was widened to state the real rule and a new test covers both halves — `marine_web_view/marine_web_view/coverage_renderer.py:1219-1230`, `test/test_render_pass.py`
- [x] PR #353's test plan claimed 161 tests / 19 mutations — updated to 208 tests (verified: full suite green with boto3 present and with boto3 forced absent) and to the round-4 record's 21/21 mutants — `PR #353 body` (edited via `gh pr edit`; no commit)

### Verification
- `python3 -m pytest test/` in `marine_web_view`: **208 passed** (was 203 before this pass — 5 new tests).
- Same suite re-run with `boto3`/`botocore` shadowed by raising stubs: **208 passed**.
- `test_flake8`, `test_pep257`, `test_copyright` included and green; pre-commit hooks ran on every commit.

### Next step
Lifecycle: **Implementation** → **review-code** (re-review the fixes)

    .agent/scripts/dispatch_subagent.sh --mode in-process --issue 351 --skill review-code
