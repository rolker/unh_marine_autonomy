...
## Implementation
**Status**: complete
**When**: 2026-08-26 03:45 +0000
**By**: Claude Code Agent (Claude Opus)

**Branch**: feature/issue-357 at `3ba0dbd`
**Addressed**: `## Local Review (Pre-Push)` of 2026-08-26 03:23 +0000, branch `feature/issue-357` at `aa76967` (verdict changes-requested, 2 must-fix + 12 suggestions)
**Commits**: `5ba6051`, `bd5699f`, `6663671`, `27f078f`, `6099854`, `f05303e`, `88594e9`, `fed0888`, `bda1fcb`, `d0282f4`, `ffdc08e`, `6ec95ce`, `21eb92e`, `3ba0dbd`

All 14 open actions were fixed; none were deferred. Every fix is its own commit, and every behavioural fix carries a test that fails without it. Local suite: **272 passed** (was 259), `xmllint package.xml` OK, `node --check` on the page script OK. No push — the host performs pushes.

### Actions
- [x] (must-fix) SAR aircraft published — `excluded_from_public` now withholds `position_message_id == 9` (`SAR_AIRCRAFT_MESSAGE_ID`), the only identifier a SAR aircraft carries: message 9 has no navigational status and an aircraft sends no type 5/24 static report, so `ship_and_cargo_type` stays 0 forever. `ais_layer.cpp` switches on the same field. README table row and prose updated; two tests, one pinning that ordinary message ids (1/2/3/18/19/27) and the bare default status 15 still publish — `marine_web_view/marine_web_view/ais_renderer.py:214-222` (`5ba6051`)
- [x] (must-fix) Distress beacon not transmitting status 14 — added `DISTRESS_MMSI_PREFIXES = {970, 972, 974}`, the ITU-reserved SART/MOB/EPIRB ranges, tested on the first three digits of the MMSI. `DISTRESS_NAV_STATUS` is now `NavigationalStatus.NAVIGATIONAL_STATUS_SART_MOB_EPIRB` rather than a retyped `14`, with a test pinning the two equal. Tests cover all three prefixes at status 15 and confirm a short or embedded lookalike MMSI still publishes — `marine_web_view/marine_web_view/ais_renderer.py:117-133,213` (`bd5699f`)
- [x] (suggestion) `pollAis`'s catch calls the function that can throw — `drawAis()` no longer trusts the artifact: `coordinates` is checked for being an array of two finite numbers before destructuring, `aisFeatures` is guarded with `Array.isArray` at both the loop and the assignment in `pollAis`. One malformed feature now costs that feature. Guard test asserts `f.geometry.coordinates` is gone and both `isFinite`/`isArray` checks are present — `marine_web_view/web/index.html:941,991` (`6663671`)
- [x] (suggestion) `vessel_dimensions` and ITU's "reference point not available" encoding — `A = C = 0` with `B`/`D` non-zero is now detected and the hull is **centred** (`length=B`, `width=D`, offsets 0) rather than drawn entirely aft of the marker. Recorded as a deliberate divergence from upstream `calculatePolygon` in the docstring and the README, with the reason: the tracker's consumers are planners that carry their own margins, this one is a picture a person reads. Test covers (0,100,0,20) and confirms a genuine bow reference (`A=0, C!=0`) keeps its offsets — `marine_web_view/marine_web_view/ais_renderer.py:245-260` (`27f078f`)
- [x] (suggestion) No own-ship / operator ignore list — added the `ignore_mmsis` parameter, same name and first purpose as `ais_layer`'s. Applied in `_on_contact` (dropped from memory, not merely from the snapshot), logged at startup, forwarded through the launch file as a typed `ParameterValue` (a bare `LaunchConfiguration` reaches the node as the string `'[]'`). Declared with `dynamic_typing` because an empty-list default infers `BYTE_ARRAY`; `_ignore_list()` is therefore the real type check and refuses a malformed list loudly rather than silently withholding nobody. `_forwarded_parameters` in `test_launch_params.py` was taught the `ParameterValue(...)` spelling rather than being relaxed — `marine_web_view/marine_web_view/ais_renderer.py:190` (`6099854`)
- [x] (suggestion) `test_the_ais_readout_cannot_be_shadowed_by_a_contact_count` did not pin what it claimed — the ordering assertion (matching the literal `'no contacts'`) is replaced by one over `aisText`'s return statements: every return carrying `count`/`body` must also carry `AIS_STALE_S` or `age unknown`. Verified by mutation — replacing the final return with a bare `return body;` fails the guard, which the old assertion did not — `marine_web_view/test/test_page_layers.py:469` (`f05303e`)
- [x] (suggestion) Unpaired-surrogate rationale false — confirmed against the interpreter (`json.dumps({'n':'TUG\ud800'})` -> `'{"n": "TUG\\ud800"}'`, encodes and parses cleanly). The claim is corrected in all three places (module comment, test docstring, README): the scrub is kept for the smaller, true reasons — not a character, renders as a replacement glyph, and one `ensure_ascii=False` away from a `UnicodeEncodeError` that would take the whole artifact — `marine_web_view/marine_web_view/ais_renderer.py:141` (`88594e9`)
- [x] (suggestion) `aisLayer` added unconditionally against a restored checkbox — the group is no longer `.addTo(map)` at construction; `syncAisLayer()` reads `$('ais').checked` and is called both at startup and from the `change` handler, the way `follow` reads its box at use time. `test_every_vector_layer_reaches_the_map` now recognises the `map.addLayer(name)` spelling (the only one with a `removeLayer` counterpart) alongside `.addTo(map)`, so the layer-reachability guard still bites — `marine_web_view/web/index.html:815,997` (`fed0888`)
- [x] (suggestion) `interval` unvalidated on the documented cost lever — `sane_interval` and `MAX_INTERVAL_SECONDS` moved from `coverage_renderer` to `renderer_common` (its stated home for exactly this), `coverage_renderer` and `test_tile_ingest` updated to the new home, and `ais_renderer` now validates `interval` against it with a `DEFAULT_INTERVAL` fallback and a warning. Test constructs the node with `interval:=0.0` — which previously raised out of `create_timer` after the upload worker was already running. `state_renderer` is left alone: pre-existing, and out of this diff's scope — `marine_web_view/marine_web_view/ais_renderer.py:373` (`bda1fcb`)
- [x] (suggestion) `_clean` had no length cap — capped at `MAX_TEXT_CHARS = 40` (20 for a name plus 14 for the AtoN extension, with headroom), truncation marked with an ellipsis so a shortened name does not read as whole. README's Output section says so — `marine_web_view/marine_web_view/ais_renderer.py:148` (`d0282f4`)
- [x] (suggestion) `heading_deg` at full float precision — rounded to 1 dp in `heading_degrees`, matching `speed_knots` and `course_deg`; AIS transmits whole degrees and the popup prints zero decimal places — `marine_web_view/marine_web_view/ais_renderer.py:537` (`ffdc08e`)
- [x] (suggestion) Zero-contact case has no liveness signal — the claim is narrowed rather than papered over, in all three places that made it (page comment beside `aisText`, `test_a_dead_ais_renderer_does_not_read_as_a_quiet_river`'s docstring, README Output). With no contacts held there is no stamp to ride on, so an empty river and a renderer that died an hour later both read as *no contacts* indefinitely; only *offline* is distinguishable. The README also says why closing it is not free (a PUT per `interval` to report nothing) and that the ROC-side log is what tells the two apart — `marine_web_view/web/index.html:872` (`6ec95ce`)
- [x] (suggestion) `AsyncUploader.submit` docstring said "the two callers" — now states the tag is genuinely opaque: two callers pass a fix stamp, the third an entire JSON signature, and nothing in the uploader reads either — `marine_web_view/marine_web_view/s3_upload.py:231` (`21eb92e`)
- [x] (suggestion) README replay recipe surprises with faded contacts — added a short paragraph under the replay recipe: the page has no sim clock, it tests `Date.now()/1000 - stamp`, so every contact from a bag older than `AIS_STALE_S` draws faded and reads *stale*. Correct behaviour, now stated — `marine_web_view/README.md` (Running) (`3ba0dbd`)

### Deferred
None. Every finding was verified against the current source before it was acted on, and all fourteen were live.

### Notes for the re-review
- The two must-fixes are the ones to read first, and both are reproducible from the tests rather than from the page.
- Three guards were widened rather than merely satisfied, each with its reason recorded in the test: `_forwarded_parameters` (the `ParameterValue` spelling), `test_every_vector_layer_reaches_the_map` (the `map.addLayer` spelling), and the readout-shadowing assertion (rewritten to pin the count).
- `sane_interval` moving to `renderer_common` touches `coverage_renderer` and `test_tile_ingest`; no behaviour changed there, only the import.
- The Simulation-First Watch is unchanged: the live pandy pass is still outstanding, and `contact_timeout`, `interval`, `max_contacts` and `AIS_STALE_S` remain unverified judgement calls.
