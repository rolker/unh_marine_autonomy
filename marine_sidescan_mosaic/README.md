...
| `--bathy-layers` | `processed,draft,reference` | Layer directory search order, highest priority first (mirrors the store's `processed > draft > reference` — ADR-0010 D8 split the pre-D8 `survey/` into `processed/`+`draft/`, so the equivalent of the old `survey,reference` default is now `processed,draft,reference`). `chart` is **not** in the default: charted soundings are cartographically shoal-biased for navigation safety, and a shoal-biased vertical term would bias placement (the store's safety query — ADR-0002 D7's shallowest-reliable mode, refined by ADR-0010 D4 — is where that bias is *wanted*); opt in only where it is the only coverage. ADR-0010 D8's `survey/`→`processed/` re-classification is a change to this flag, not to code — and a requested layer that is missing only **warns** if another requested layer still has tiles. A list naming **no** layer at all (empty, or all separators) is an argument error (exit 2), not a fall-back to the default |
